...
const unstake = async (federation_account, token_account, account, planet_name, quantity, eos_api) => {
  ...
  const planet_res = await eos_api.rpc.get_table_rows({
    code: federation_account,
    scope: federation_account,
    table: 'planets',
    limit: 1,
    lower_bound: planet_name,
    upper_bound: planet_name
  });
...
const getMap = async (federation_account, planet_name, eos_api) => {
  const res = await eos_api.rpc.get_table_rows({
    code: federation_account,
    scope: planet_name,
    table: 'maps',
    limit: 1000
